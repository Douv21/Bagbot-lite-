...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('travailler')
    .setDescription('Travailler avec quelqu\'un')
  ...
  async execute(interaction) {
    ...
    const actionConfig = config?.actions?.commands?.['travailler'] || { enabled: true, rewardMin: 5, rewardMax: 15, karmaMin: 1, karmaMax: 3, messages: [] };
    ...
    if (actionConfig.messages && actionConfig.messages.length > 0) {
      ...
      message = `${author} travaille avec ${target} !`;
    }

    const embed = new EmbedBuilder()
      .setTitle('⚒️ Travailler')
