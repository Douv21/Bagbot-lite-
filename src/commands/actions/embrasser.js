...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName('cible').setDescription('Personne à embrasser (optionnel)').setRequired(false))
  ...
  async execute(interaction) {
    ...
    const actionConfig = config?.actions?.commands?.['embrasser'] || { enabled: true, rewardMin: 5, rewardMax: 15, karmaMin: 1, karmaMax: 3, messages: [] };
    ...
    if (actionConfig.messages && actionConfig.messages.length > 0) {
      ...
      message = `${author} embrasse ${target} !`;
    }
