...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('vin')
    .setDescription('Offrir du vin à quelqu\'un')
  ...
  async execute(interaction) {
    ...
    const actionConfig = config?.actions?.commands?.['vin'] || { enabled: true, rewardMin: 5, rewardMax: 15, karmaMin: 1, karmaMax: 3, messages: [] };
    ...
    if (actionConfig.messages && actionConfig.messages.length > 0) {
      ...
      message = `${author} offre du vin à ${target} !`;
    }

    const embed = new EmbedBuilder()
      .setTitle('🍷 Offrir du vin')
