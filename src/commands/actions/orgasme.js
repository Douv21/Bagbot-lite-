...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('orgasme')
    .setDescription('Faire jouir quelqu\'un')
  ...
  async execute(interaction) {
    ...
    const actionConfig = config?.actions?.commands?.['orgasme'] || { enabled: true, rewardMin: 5, rewardMax: 15, karmaMin: 1, karmaMax: 3, messages: [] };
    ...
    if (actionConfig.messages && actionConfig.messages.length > 0) {
      ...
      message = `${author} fait jouir ${target} !`;
    }

    const embed = new EmbedBuilder()
      .setTitle('🌊 Faire jouir')
