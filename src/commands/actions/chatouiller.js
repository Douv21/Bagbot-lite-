...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const actionConfig = config?.actions?.commands?.['chatouiller'] || { enabled: true, rewardMin: 5, rewardMax: 15, karmaMin: 1, karmaMax: 3, messages: [] };
    ...
    if (actionConfig.messages && actionConfig.messages.length > 0) {
      ...
      message = `${author} chatouille ${target} !`;
    }

    const embed = new EmbedBuilder()
      .setTitle('😂 Chatouiller')
