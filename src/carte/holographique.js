...
// Gem color sets per rank  [highlight, mid, dark]
...
module.exports = async (member, data) => {
  ...
  // Aurora shimmer — top left
  const a1 = ctx.createRadialGradient(120, 80, 0, 120, 80, 420);
  a1.addColorStop(0,   "rgba(80,40,180,0.55)");
  a1.addColorStop(0.3, "rgba(0,120,200,0.35)");
  a1.addColorStop(0.6, "rgba(0,200,180,0.15)");
  a1.addColorStop(1,   "rgba(0,0,0,0)");
  ctx.fillStyle = a1; ctx.fillRect(0, 0, W, H);

  // Aurora shimmer — top right
  const a2 = ctx.createRadialGradient(W - 200, 100, 0, W - 200, 100, 500);
  a2.addColorStop(0,   "rgba(140,0,200,0.45)");
  a2.addColorStop(0.25,"rgba(0,100,220,0.35)");
  a2.addColorStop(0.5, "rgba(0,200,255,0.20)");
  a2.addColorStop(1,   "rgba(0,0,0,0)");
  ctx.fillStyle = a2; ctx.fillRect(0, 0, W, H);

  // Aurora shimmer — bottom accent
  const a3 = ctx.createRadialGradient(W * 0.5, H, 0, W * 0.5, H, 300);
  a3.addColorStop(0,   "rgba(0,80,160,0.30)");
  ...
  // Corner brackets (decorative L-shapes)
  const bLen = 38, bT = 3;
  const corners = [
  ...
  // Iridescent ring (rainbow gradient)
  const ringGrad = ctx.createLinearGradient(ax - ar - 12, ay - ar - 12, ax + ar + 12, ay + ar + 12);
  ringGrad.addColorStop(0,    "#ff88cc");
  ringGrad.addColorStop(0.18, "#cc88ff");
  ringGrad.addColorStop(0.36, "#88aaff");
  ringGrad.addColorStop(0.50, "#00eeff");
  ringGrad.addColorStop(0.64, "#88ffcc");
  ringGrad.addColorStop(0.80, "#cc88ff");
  ringGrad.addColorStop(1,    "#ff88cc");

  ctx.beginPath(); ctx.arc(ax, ay, ar + 14, 0, Math.PI * 2);
  ctx.strokeStyle = ringGrad; ctx.lineWidth = 14;
  ctx.shadowColor = "#60c0ff"; ctx.shadowBlur = 22;
  ctx.stroke(); ctx.shadowBlur = 0;

  // Tick marks on ring (4 diagonal positions)
  ...
  // Username
  let namePx = 68;
  ctx.font = `bold ${namePx}px Orbitron, Arial`;
  while (ctx.measureText(username).width > 480 && namePx > 30) {
    namePx -= 2;
    ctx.font = `bold ${namePx}px Orbitron, Arial`;
  }
  ctx.fillStyle = "#ffffff";
  ctx.shadowColor = "#a0c8ff"; ctx.shadowBlur = 16;
  ctx.fillText(username, nameX, 220); ctx.shadowBlur = 0;

  // Discriminator
  ctx.font = "30px Orbitron, Arial"; ctx.fillStyle = "#6080a0";
  const discrimText = discrim && discrim !== "0" ? `#${discrim}` : "#0000";
  ctx.fillText(discrimText, nameX + 2, 268);
  ...
  nbg.addColorStop(0, "rgba(10,20,60,0.92)");
  nbg.addColorStop(1, "rgba(5,10,40,0.92)");
  ctx.fillStyle = nbg; ctx.fill();

  // Panel border with angular corners (targeting reticle style)
  const nBLen = 50;
  ...
  [[NX, NY],[NX+NW,NY],[NX,NY+NH],[NX+NW,NY+NH]].forEach(([cx,cy],i)=>{
    const [dx,dy] = dirs[i];
    ctx.beginPath();
    ctx.moveTo(cx+dx*nBLen,cy); ctx.lineTo(cx,cy); ctx.lineTo(cx,cy+dy*nBLen);
    ctx.stroke();
  });
  ctx.shadowBlur = 0;

  // "NIVEAU" label + star decorations
  ...
  // Big level number — holographic rainbow gradient
  ...
  lvlGrad.addColorStop(0,    "#e0f0ff");
  lvlGrad.addColorStop(0.15, "#a0b8ff");
  lvlGrad.addColorStop(0.30, "#c080ff");
  lvlGrad.addColorStop(0.48, "#ff80cc");
  lvlGrad.addColorStop(0.64, "#80d8ff");
  lvlGrad.addColorStop(0.80, "#a0e8cc");
  lvlGrad.addColorStop(1,    "#e0f8ff");
  ctx.fillStyle = lvlGrad;
  ctx.shadowColor = "#60a8ff"; ctx.shadowBlur = 50;
  ...
  if (pct > 0) {
    ...
    fg.addColorStop(0,    "#ff80cc");
    fg.addColorStop(0.18, "#cc80ff");
    fg.addColorStop(0.36, "#80aaff");
    fg.addColorStop(0.52, "#00eeff");
    fg.addColorStop(0.70, "#80ffcc");
    fg.addColorStop(0.86, "#cc80ff");
  ...
  const msgStr   = messages >= 10000 ? `${Math.floor(messages/1000)}K`
                 : messages >= 1000  ? `${(messages/1000).toFixed(1)}K`
                 : String(messages);
  ...
  statsData.forEach((s, i) => {
    ...
    ctx.font = "26px Arial"; ctx.fillStyle = "#a0d0ff";
    ctx.fillText(s.icon, sx, PY + 74);
    ctx.font = "16px Orbitron, Arial"; ctx.fillStyle = "rgba(120,180,255,0.7)";
  ...
  ctx.font = "italic 14px Orbitron, Arial";
  ctx.fillStyle = "rgba(100,160,255,0.38)";
  ...
  ctx.fillStyle = nlG;
  ctx.fillText(`NIVEAU ${nextLvl}`, p3x + 98, PY + 118);
...
// ─── Crystal gem (3D faceted diamond) ────────────────────────────────────
